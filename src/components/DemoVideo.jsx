 ...
 *
 * `variant`:
 *   "cover" — the whole poster is the button (used in contained panels)
 *   "fill"  — the poster fills its container and the play control is a discrete
 *             positioned pill, so it cannot sit on top of the band's copy and
 *             swallow clicks meant for the project link
 *
...
export default function DemoVideo({
  ...
  const failure = (
    <div className="absolute inset-0 z-10 grid place-items-center bg-black/70 p-6 text-center">
      <div>
  ...
  const pill = (
    <span className="flex items-center gap-2.5 rounded-full border border-white/25 bg-black/65 px-5 py-3 text-[15px] font-medium text-white backdrop-blur-sm transition-transform duration-200 group-hover:scale-[1.04]">
      ...
      <div className="relative h-full w-full">
        ...
        {failed ? (
          ...
          <button
            ...
            className={`group absolute z-10 ${controlClassName}`}
          >
            {pill}
